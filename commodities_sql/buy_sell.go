...
func BuyCommodity(ctx context.Context, pool *pgxpool.Pool, userID int, symbol string, usdAmount float64, price float64) (float64, error) {
	...
	err = tx.QueryRow(ctx, `SELECT quantity, avg_cost FROM commodity_holdings WHERE user_id = $1 AND symbol = $2 FOR UPDATE;`, userID, symbol).Scan(&existingQty, &existingAvg)
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO commodity_holdings (user_id, symbol, quantity, avg_cost, updated_at)
		VALUES ($1, $2, $3, $4, now())
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO commodity_history (user_id, symbol, operation_type, usd_amount, quantity, price)
...
func SellCommodity(ctx context.Context, pool *pgxpool.Pool, userID int, symbol string, usdAmount float64, price float64) (float64, float64, error) {
	fundingSource, err := bankcards_sql.ResolveFundingSource(ctx, pool, userID)
	if err != nil {
		return 0, 0, err
	}
	...
	if remainingQty <= 0.00000001 {
		_, err = tx.Exec(ctx, `DELETE FROM commodity_holdings WHERE user_id = $1 AND symbol = $2;`, userID, symbol)
	} else {
		_, err = tx.Exec(ctx, `UPDATE commodity_holdings SET quantity = $1, updated_at = now() WHERE user_id = $2 AND symbol = $3;`, remainingQty, userID, symbol)
	}
	if err != nil {
		return 0, 0, err
	}
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO commodity_history (user_id, symbol, operation_type, usd_amount, quantity, price, realized_pnl)
