...
var Symbols = []string{"GC=F", "SI=F", "CL=F", "BZ=F", "NG=F", "HG=F", "PL=F", "ZC=F", "ZW=F", "KC=F"}
...
func Start() {
	...
	log.Println("commodity oracle started (Yahoo Finance, same scheme as stocks)")
}
...
func fetchOne(symbol string) (Quote, bool) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		log.Println("commodity oracle: non-200 for", symbol, resp.StatusCode, string(body))
		return Quote{}, false
	}
